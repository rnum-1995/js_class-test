...
document.addEventListener('DOMContentLoaded', () => {
  ...
  departmentFilter.addEventListener('change', () => {
    if (value = "") {
      ...
      let developer = employees.filter(e => e.department === "開発部")
      renderTable(developer);
    } else if (value = "インフラ部") {
      let infra = employees.filter(e => e.department === "インフラ部")
      renderTable(infra);
    } else if (value = "デザイン部") {
      let design = employees.filter(item => item.department === "デザイン部")
      renderTable(design);
    }
  });
...
function showDetail(id) {
  ...
  const dialog = document.getElementById('detail-modal');
  let targetId = employees.find((employee) => {
    return employee.id === id;
  });

  document.getElementById('modal-image').src = `./images/${targetId.image}`;
  ...
  document.getElementById('modal-birth').innerText = age;
  document.getElementById('modal-dept').innerText = targetId.department;
  document.getElementById('modal-licenses').innerText = targetId.licenses;
...
let age = calculateAge('2000-02-04');
