...
document.addEventListener('DOMContentLoaded', () => {
  ...
  filter.addEventListener('change', () => {
    ...
    if (dept) {
      // ★ TODO: 部署での絞り込みロジックを確認
      const filtered = employees.find(emp => emp.department === dept);
...
function renderTable(list = employees) {
  ...
  list.forEach(emp => {
    ...
    tr.innerHTML = `
            ...
            <td>${age}歳</td>
            <td>
                <!-- ★ TODO: イベントハンドラの設定(ボタンをクリックしたらshowDetail()関数が動くようにする) -->
...
function showDetail(id) {
  console.log("クリックされた引数:", id); // デバッグ用ログ

  const emp = employees.find(e => e.id === id);
  if (!emp) {
    console.error("社員が見つかりません。IDを確認してください:", id);
    alert("エラー: 社員データが見つかりません");
    return;
  }
